...
Built across five milestones on branch
`robot-army/issue-321-add-a-cash-position-page-showing-the`.

| Milestone | Delivered |
|---|---|
| M1 | `budget_accounts` association table, `Budget.accounts` / `Account.budgets`, migration `2d881fa466fe` (up and down tested), per-revision migration test |
| M2 | `biweeklybudget/cashposition.py`; `NotificationsController` refactored to delegate; 114 new unit tests |
| M3 | `/cash-position`, `CashPositionView`, `cash-position.html`, nav entry, banner link; 28 new acceptance tests |
| M4 | Budget modal "Held in accounts" checkboxes, `BudgetFormHandler` link handling, sample data links; 10 new acceptance tests |
| M5 | `app_usage.rst` section, three API stubs, screenshot entry, version 1.12.0, `CHANGES.rst` |

**Decisions taken during implementation, and why:**

- **`CashPosition` computes lazily, not eagerly.** The plan said eagerly.
  The existing `TestCreditAccountSum` unit tests pass a `Mock` session and
  patch only `Account.active_credit_accounts`; eager construction would have
  run every other query against that Mock and broken tests that have nothing
  to do with what changed. Lazy evaluation also means the banner pays for the
  five figures it reads and nothing more. A test now pins that constructing a
  `CashPosition` runs no queries at all. The contract was updated to record
  this.
- **The five `NotificationsController` methods resolve `sess=None` against
  `notifications.db_session` before delegating.** Not cosmetic: the existing
  tests patch that name, and resolving inside `CashPosition` instead would
  have made them fail.
- **`Budget.account_ids` is declared in `_dict_properties`.** `as_dict` is
  built from `vars(self)`, so a relationship appears there only when it
  happens to have been loaded; without this the modal's checkboxes would have
  been intermittently wrong rather than reliably wrong.
- **Coverage groups with no active standing budget are not emitted**; their
  accounts are reported as unlinked instead, so every account appears in
  exactly one place on the page. An account linked *only* to inactive budgets
  therefore counts as unlinked, because an inactive budget allocates nothing.
- **`docs/source/screenshots.rst` was not hand-edited.** It is generated by
  `docs/make_screenshots.py`; the new page was added to that script's list
  instead, so the entry and its image are produced by `tox -e screenshots` at
  release time rather than referencing an image that does not exist yet.

**Requirements coverage**: FR-001 through FR-025 are all implemented. No
requirement was dropped, deferred, or reinterpreted.
