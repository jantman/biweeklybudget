...
- [ ] T001 Start the MariaDB 10.4.7 test container and create the test databases per `CLAUDE.md` ("Test Database Setup for Development"). Then confirm the unmodified `biweeklybudget/tests/unit/test_plaid_updater.py` passes, with output redirected to a scratchpad file, to get a green baseline.
...
- [ ] T002 [US1] In `biweeklybudget/tests/unit/test_plaid_updater.py`:
...
- [ ] T003 [US1] Run the T002 tests and confirm they fail (unexpected `negate_balance` keyword / positive values).
- [ ] T004 [US1] In `biweeklybudget/plaid_updater.py` (see plan "Design" and `contracts/plaid-loan-balance.md`):
...
- [ ] T006 [P] In `docs/source/plaid.rst`, add a `Loan Accounts` subsection (`~~~~` underline) at the end of the `Usage` section, before `Troubleshooting`, with an RST label `.. _plaid.loan_accounts:`. It should state that:
