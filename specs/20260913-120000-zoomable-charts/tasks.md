...
- [X] T031 *(Result, 2026-09-13: PR [#341](https://github.com/jantman/biweeklybudget/pull/341) opened. First CI run: every check passed (py314, docs, jsdoc, screenshots, acceptance, plaid, migrations, docker, coverage, claude-review, snyk); `claude` (the @claude-mention job) skipped as expected. The `docker` job is the Docker suite run recorded under T029. Review round 1: Claude found that each donut redraw leaked a Chart.js instance. `budgetSpendingDrawPanel()` emptied the container first, so `budgetSpendingDrawChart()` could no longer find the old canvas to destroy it. Fixed by leaving the container to `budgetSpendingDrawChart()`. `test_toggle_redraws_one_chart_each` now also checks that the number of live Chart.js instances (`Chart.instances`) equals the number of charts shown, a check that failed on the previous code. The github-actions coverage report (50.9%) is the unit-test figure, unchanged from #340; no Python changed. Both were replied to on the PR, and the review thread was resolved. Review round 2, on 0cb85f2: Claude's review reported "No issues found". No Copilot review: Copilot is not a requested reviewer on this repository, and #338 and #340 have none. CI on 0cb85f2: every check passed (py314, docs, jsdoc, screenshots, acceptance, plaid, migrations, docker, coverage, claude-review, snyk).)* Mark tasks complete, set the spec's Status to Complete, and commit everything as `Zoomable Charts - M2.n`. Push the branch to `origin` and open the pull request following `.github/PULL_REQUEST_TEMPLATE.md`. Surface for the maintainer the library choice (and why not Bokeh), the Ctrl modifier, the donuts moving too, and the ~290 KB of vendored JS. Then monitor CI and answer reviews until Claude's review says "No issues found" and Copilot's, if present, recommends approval.
