...
- [ ] T001 Start the MariaDB test container and export the test environment variables per `CLAUDE.md` ("Test Database Setup for Development"). Then run `python dev/setup_test_db.py` and `initdb`, using the main checkout's venv (worktrees have no venv). No migration is created by this feature.
- [ ] T002 Run the existing chart-related acceptance tests on the unchanged code (`tox -e acceptance -- -k "Chart or chart or BudgetSpending or Fuel"`, output to a scratchpad file) to have a green baseline to compare against.
...
- [ ] T025 Delete `static/startbootstrap-sb-admin-2/vendor/morrisjs/` and `static/startbootstrap-sb-admin-2/vendor/raphael/`. Then grep the repository, excluding `specs/`, for `morris`, `Morris` and `raphael`, and remove or update every remaining reference.
- [ ] T026 [P] In `docs/make_screenshots.py`, replace the `.morris-hover` script in the Budgets pre-shot with a script that shows a tooltip on the per-period chart via `Chart.getChart('budget-per-period-chart-canvas')`, `tooltip.setActiveElements(...)` and `update()`.

---

## Phase 8: Polish, Docs & Test Gate (M2)

- [ ] T027 [P] Add a concise entry under `Unreleased` at the top of `CHANGES.rst`, led by the issue #215 link, with sub-bullets for the new controls and the Morris → Chart.js replacement. Do not touch `biweeklybudget/version.py`.
- [ ] T028 [P] Check `README.rst`, `CLAUDE.md` and `docs/source/` for anything else that names Morris or describes chart behaviour, and update it. Mention the chart controls in the Spending Charts section of `app_usage.rst` only if something there changed.
- [ ] T029 Run the Test Gate to completion with output to scratchpad files: `tox -e py314`, `tox -e acceptance`, `tox -e migrations`, `tox -e docs`, `tox -e jsdoc` (jsdoc 4.0.4; commit the regenerated `docs/source/jsdoc.*.rst`, including the new `jsdoc.charts.rst`, and add it to whatever toctree lists the others). Raise timeouts rather than narrowing any run. Re-run known-flaky tests in isolation before attributing a failure. Record the results in this file.
- [ ] T030 Manually walk `quickstart.md`'s scenarios against `flask rundev` with the test data, in a browser, and fix anything found.
