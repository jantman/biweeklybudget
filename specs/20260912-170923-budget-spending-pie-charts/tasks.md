...
- [X] T021 *(Results, 2026-09-12, on the final code against a fresh MariaDB 10.4.7 test DB: `py314` 955 passed, 4 skipped (pre-existing; cache cleared so no pycodestyle/pyflakes check was skipped); full `acceptance` 824 passed, 24 skipped (pre-existing: 6 deprecated OFX, 18 unconditional reconcile), 1 failed: `test_base_template.py::test_nav_links`, a sidebar-list assertion missed in T016, fixed in M1.3, after which `test_base_template.py` passed 33/33; `migrations` 8 passed; `docs` OK, no broken links; `jsdoc` OK with jsdoc 4.0.4 as in CI. An earlier full run was stopped and discarded when the manual walkthrough (T022) found the chart-width bug fixed in M1.2.)* Run the Test Gate to completion, output to scratchpad files: `tox -e py314`, `tox -e acceptance`, `tox -e migrations`, `tox -e docs`, `tox -e jsdoc` (commit the generated `docs/source/jsdoc.budget_spending.rst`). Raise timeouts rather than narrowing any run. Re-run known-flaky tests in isolation before attributing a failure. Record the results in this file.
