...
* `Issue #311 <https://github.com/jantman/biweeklybudget/issues/311>`_ - Remove the ``sqlalchemy-diff`` test dependency. The migration tests now compare the migrated database to the models using Alembic's ``compare_metadata()``, and use alembic-verify 1.x's supported fixtures.
  ...
  * The second migration test database is no longer used, and the ``MYSQL_DBNAME_RIGHT`` environment variable is removed.
  * The comparison now also checks column server defaults. It no longer checks CHECK constraints, all of which the previous comparison ignored.

* `Issue #321 <https://github.com/jantman/biweeklybudget/issues/321>`_ - Add a **Cash Position** page (``/cash-position``), linked from the sidebar and from the unallocated-funds notification, showing the notification's calculation as a full waterfall with ledger and projected balances per account.

  * Standing budgets can now be linked to the accounts holding their funds, using checkboxes on the budget modal; the page uses these links to report differences between account balances and the standing budgets they hold.
  * Database migration ``2d881fa466fe`` adds the ``budget_accounts`` association table.

* `Issue #320 <https://github.com/jantman/biweeklybudget/issues/320>`_ - Fix the unallocated-funds notification overstating available funds by ignoring credit card balances. Also closes `Issue #209 <https://github.com/jantman/biweeklybudget/issues/209>`_.

  * Active credit account balances are now subtracted from available funds; an overpaid card (positive balance) increases them.
  * The pay period figure is relabeled from "remaining" to "allocated but unspent", as it is not the same figure as the pay period view's "remaining".

* `Issue #322 <https://github.com/jantman/biweeklybudget/issues/322>`_ - ``POST /forms/transaction`` now accepts Account and Budget names (exact, case-insensitive) as well as IDs, and add an ``addtrans`` console script that creates Transactions through it.

  * Values that are all digits are looked up as IDs first, then as names.
  * ``notes`` and ``sales_tax`` are now optional, as documented; omitting them previously caused an HTTP 500.
  * Validation errors ``Account ID %s is invalid.`` and ``Budget ID %s is invalid.`` are reworded to ``Account "%s" is invalid.`` and ``Budget "%s" is invalid.``.

* `Issue #279 <https://github.com/jantman/biweeklybudget/issues/279>`_ - Make the index page Account Balances chart much faster by limiting it to a configurable time window, and add a range selector (1 month to all history).
...
* `Issue #323 <https://github.com/jantman/biweeklybudget/issues/323>`_ - Currency inputs throughout the UI now accept common formatting, such as ``1,234.56``, ``123``, ``$123`` and ``(12.34)``, which previously caused errors or were rejected. Ambiguous values such as ``10,00`` are rejected with a validation error.
