...
* `Issue #311 <https://github.com/jantman/biweeklybudget/issues/311>`_ - Remove the ``sqlalchemy-diff`` dependency and move the migration tests onto alembic-verify's supported interface.

  * **The problem.** ``alembic-verify`` began life inside the ``sqlalchemy-diff`` project, and this project carried both, pinned together, because getting a working combination required it. ``alembic-verify`` 1.0 severed that relationship entirely - it no longer depends on ``sqlalchemy-diff``, and it renamed its fixtures away from the "left database / right database" vocabulary that only ever existed to serve the schema-comparison use case. 1.6.2 took the version bump but left both halves of the coupling in place: ``sqlalchemy-diff`` survived as a direct dependency of this project's own test code, and the migration fixtures still carried the deprecated names. That is one extra dependency to pin, audit and keep compatible with the SQLAlchemy version the application actually uses, for the sake of a single assertion in a single test.
  * ``sqlalchemy-diff`` is **removed from all four tox environments** that declared it, and from the test code. Nothing installs it and nothing imports it.
  * **The schema comparison is rebuilt on Alembic's own autogenerate machinery.** ``sqlalchemydiff.comparer.Comparer`` is replaced by ``alembic.autogenerate.compare_metadata()``, run against an ``alembic.migration.MigrationContext`` on the migrated database and compared to ``Base.metadata``. Alembic is already a hard runtime dependency, so this removes a dependency without adding one - swapping ``sqlalchemy-diff`` for a different third-party schema-diff library would have satisfied the letter of the issue and missed its point.
  * This is also the same code that backs ``alembic revision --autogenerate``, so a reported difference is now literally the migration that would need to be written. It is a more useful failure message than a structural diff, and it means the test asks exactly the question a maintainer cares about.
  * **The comparison is stricter than the one it replaces, not laxer.** It runs with ``compare_server_default=True``, which the previous mechanism did not do, so a migration that creates a column with the wrong server default is now caught. It needs **no ignore list at all**: the ``alembic_version`` table is excluded by Alembic automatically, and the sixteen named check constraints the old comparison had to ignore - ``accounts``, ``budgets``, ``ofx_trans``, ``reconcile_rules`` and ``scheduled_transactions`` - are not needed either. Measured against this schema, it reports zero differences.
  * **One nominal narrowing, recorded in the test itself**: Alembic's autogenerate does not examine CHECK constraints, so differences in them are neither detected nor reported. Nothing that was actually being verified stops being verified - the previous comparison had every one of this schema's check constraints explicitly listed in its ``ignores`` argument, because they did not diff correctly.
  * **The comparison was proven non-vacuous before being trusted.** A test that passes because it compared nothing is worse than no test. Injecting each of six kinds of model/migration drift - a column present only in the models, a changed column type, a changed nullability, a table present only in the migrations, an index present only in the models, a foreign key present only in the migrations - produces a failure naming the offending element in every case.
  * **The second ("right") test database is gone.** It existed solely to give ``sqlalchemy-diff`` a second live schema to inspect; ``compare_metadata`` compares a live database against in-memory metadata, so the models no longer need materializing. ``MYSQL_DBNAME_RIGHT`` is no longer read by anything and is removed from ``dev/setup_test_db.py``, the CI workflow, ``CLAUDE.md`` and the documentation. The local ``prepare_schema_from_models()`` helper added in 1.6.2 is deleted.
  * **The fixtures now use alembic-verify 1.x's supported names**: ``uri_left`` becomes ``alembic_db_uri``, ``alembic_config_left`` becomes ``alembic_config``, and ``uri_right`` and ``alembic_config_right`` are removed. They move out of ``biweeklybudget/tests/conftest.py`` - which the unit and acceptance suites also load - and into the migration suite's own conftest, where they belong. This project still supplies its own ``alembic_config`` rather than using the library's, because the library's resolves ``script_location`` out of ``alembic.ini`` as a path relative to the working directory; ours uses the absolute path from ``alembic_root``.
  * ``MYSQL_DBNAME_LEFT`` **keeps its name.** It is still in use, so removing it was never on the table, and renaming it would silently break every existing local development environment for a cosmetic gain. The documentation now describes it as the database the migration suite builds, rather than as the first of a pair.
  * ``alembic-verify`` **keeps its exact version pin**, at the current latest ``1.0.2``. Every other test dependency in ``tox.ini`` carries an exact pin; dropping this one would make the migration environment the only one whose dependency set can drift between CI runs.
  * No application code, model, or migration changes.
